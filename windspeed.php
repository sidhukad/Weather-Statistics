...
$qry = "select MAX(WindSpeed) as 'MaxWind' from climate1 where State='$state' and Year='$year' group by Month ";
...
$qry = "select MIN(WindSpeed) as 'MinWind' from climate1 where State='$state' and Year='$year' group by Month";
$result=mysql_query($qry,$con) or die("\nError in execution: ".mysql_error());
$rows1 = array();
$rows1['name'] = 'Minimum' ;
while($rr = mysql_fetch_assoc($result)) {
	$rows1['data'][] = $rr['MinWind'] ;
}
...
<div class="container">
      ...
      <div class="masthead">
        ...
        <ul class="nav nav-justified">
          ...
          <li class="active"><a href="windspeed.php?state=<?php echo $state?>&year=<?php echo $year?>&button=submit">Wind Speed</a></li>
          <li><a href="snowdepth.php?state=<?php echo $state?>&year=<?php echo $year?>&button=submit">Snow Depth</a></li>
        </ul>
      </div>
